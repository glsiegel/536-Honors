...
public class Parser {
	...
	public Parser(List<Token> tokenList) {
		...
		this.errors = new ArrayList<error.IError>();
	...
	/**
	 * Skips tokens to a specified bracket depth in an attempt at error recovery.
	 * Requires either a rising or falling edge to stop. Meaning, if we are at
	 * bracket depth 2 and we want to skip to 2, we will consume tokens until depth
	 * is non-2 before it is 2 again, then stop.
	 * 
	 * @param token                erroring token
	 * @param target_bracket_depth bracket depth to stop consuming
	 * @param details              error details
	 * @throws ParserSoftException exception that is caught by the parser to perform
	 *                             the skipping
	 */
...
//				details);
	...
	/**
	 * Skips tokens to a specified paren depth in an attempt at error recovery.
	 * Requires either a rising or falling edge to stop. Meaning, if we are at paren
	 * depth 2 and we want to skip to 2, we will consume tokens until depth is non-2
	 * before it is 2 again, then stop.
	 * 
	 * @param token              erroring token
	 * @param target_paren_depth paren depth to stop consuming
	 * @param details            error details
	 * @throws ParserSoftException exception that is caught by the parser to perform
	 *                             the skipping
	 */
...
//				details);
	 ...
	 * else. If you are using this, really ask yourself if it's a good idea.
	 * 
	 ...
	 * the next. Only used for errors. If you are using this for something else,
	...
	private Stmt statement() {
		...
		if (match(Token.Type.IMPORT)) {
			if (!as_listed) {
				skip_line_error(peek(),
						"When using imports, must declare who this file is at the top with an 'as' statement.");
			}
			if (imports_done) {
			...
			return new Stmt.Import(new Rootpath(rootpath), id.lexeme, new CodeSection(import_token, last_token),
					rootpath_section, new CodeSection(id));
		...
		if (match(Token.Type.DO)) {
			...
			if (match(Token.Type.EOF)) {
				// we don't have end
				section = new CodeSection(do_token);
			} else {
				// we have end
				...
				section = new CodeSection(do_token, end_token);
			}
		...
		if (match(Token.Type.IDENTIFIER)) {
			if (matchNext(Token.Type.DOT)) {
				...
				if (tail_end instanceof Expr.Variable var) {
					return new Expr.Indirect(start_token.lexeme, var, new CodeSection(start_token, var.section),
							source_section);
				} else if (tail_end instanceof Expr.Call call) {
					return new Expr.Indirect(start_token.lexeme, call, new CodeSection(start_token, call.section),
							source_section);
				} else {
					// this is a problem
	...
	/**
	 * The parser determines whether the file declares itself as non-singleton.
	 * 
	 * @return true iff the file is non-singleton
	 */
	...
	/**
	 * The parser determines the rootpath of this file.
	 * 
	 * @return the rootpath, given that this file is non-singleton
	 */
