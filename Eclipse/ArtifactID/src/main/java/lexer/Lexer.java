...
public class Lexer {
	...
	/**
	 * List of all keywords known to lexer.
	 */
	private static final String[] keywords = { "int", "bool", "fun", "if", "else", "while", "return", "print", "true",
			"false", "import", "as" };
	/**
	 * Not exactly a set of operators, but if we encounter one of these, then we
	 * know an operator has started. We can use additional logic to determine what
	 * the next character ought to be.
	 */
	private static final char[] op_starters = { '+', '-', '*', '/', '>', '<', '=', '!', '&', '|' };

	/**
	 * Lexer errors are put here.
	 */
	public List<IError> errors;
	/**
	 * Generated tokens are put here.
	 */
	public List<Token> tokenList;
	/**
	 * The lexer uses this value to store its guess of whether the programmer
	 * intended the file to be a singleton or not. If an 'as' is encountered, then
	 * this is set to false.
	 */
	public boolean singleton = true;

	/**
	 * Runs the lexer on a string of the file contents. Useful for when a file's
	 * contents might not be saved.
	 * 
	 * @param contents file content string
	 */
	public void runOnString(String contents) {
		...
		try (PleasantReader reader = new PleasantReader(contents, false)) {
			...
			// hmm
			errors.add(new LexerError(CodeSection.ZERO,
					"Could not find the file when reading from text? Something horrible happened.\n" + e.getMessage()));
			tokenList = null;
		} catch (IOException ioe) {
			errors.add(new LexerError(CodeSection.ZERO,
					"Encountered IO problem when reading from text? Something horrible happened.\n"
							+ ioe.getMessage()));
			tokenList = null;
		} catch (Exception e) {
			errors.add(new LexerError(CodeSection.ZERO, "Had some inexplicable error.\n" + e.getMessage()));
			tokenList = null;
		}
	}

	/**
	 * Runs the lexer on a file.
	 * 
	 * @param f file to run on
	 */
	...
	/**
	 * Runs the lexer on a file based on the file's name.
	 * 
	 * @param file_name name of file
	 */
	public void runOnFile(String file_name) {
		if (file_name == null || file_name.length() < 1) {
			errors.add(new LexerError(CodeSection.ZERO, "No file name provided..."));
			tokenList = null;
			return;
		}
	...
	/**
	 * Indicates whether an integer represents an alphabetical character.
	 * 
	 * @param c int representation of a character
	 * @return true iff alphanumeric
	 */
	...
	/**
	 * Indicates whether an integer represents an alphabetical character, plus
	 * numbers and underscore.
	 * 
	 * @param c in representation of a character
	 * @return true iff alphabetical, numeric, or underscore
	 */
	...
	/**
	 * Generates a LexerError at the current token.
	 * 
	 * @param line    line of token
	 * @param offset  char offset of token
	 * @param token   string represenation of token
	 * @param details details about error
	 */
	 ...
	 * @return the token
	 * @throws IOException if the reader messes up
	 */
	private Token getNextToken(PleasantReader reader) throws IOException {
		while (true) { // we have this while loop, so if we encounter an invalid token, we just ignore
						// it.
