...
public class Pipeline {
	...
	public static StatementData getStatements(String contents) {
		...
		// aggregate errors
		List<error.IError> aggregate = new ArrayList<error.IError>();
	...
	/**
	 * Runs a static check on the file. Used by the interpreter prior to
	 * interpreting.
	 * 
	 * @param f file to check
	 * @return outcome, either StatementData if it's a singleton/failed early, or a
	 *         FileManager if it's not a singleton
	 */
	public static Outcome runStaticCheck(File f) {
		...
		// not singleton
		FileManager<VarType> manager;
		try {
			manager = new FileManager<>(data.rootpath, f);
		} catch (FileManagerException e) {
			error_aggregate.add(new ParserError(new CodeSection(0,0,0,0), String.format("Name '%s' did not match file's actual name.", data.rootpath)));
			StatementData our_last_data = new StatementData(data.statements, error_aggregate);
			return new Outcome(our_last_data);
		}
		...
		error_aggregate.addAll(gtb.visitorErrors);
		...
		Set<Rootpath> checked_rootpaths = new HashSet<Rootpath>();
		...
		while (!rootpaths_to_pull.isEmpty()) {
			...
			if (fileinfo == null) {
				error_aggregate.add(new error.VisitorError(new CodeSection(0, 0, 0, 0),
			...
			curr_errors.addAll(curr_gtb.visitorErrors);
			
			// now, upon checking this file, it introduces its own set of dependencies that must be checked.
			for (Rootpath rootpath_to_add : curr_gtb.rootpaths_to_pull_in) {
				if (!checked_rootpaths.contains(rootpath_to_add)) {
					checked_rootpaths.add(rootpath_to_add);
					rootpaths_to_pull.add(rootpath_to_add);
				}
			}
		...
		for (Iterator<Map.Entry<Rootpath, FileInfo<VarType>>> iterator = manager.uncheckedIterator(); iterator
				.hasNext();) {
