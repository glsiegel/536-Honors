...
import interpreter.InterpreterPipeline;
import pipeline.FileManagerSet;
...
public class BadlangLanguageServer implements org.eclipse.lsp4j.services.LanguageServer, LanguageClientAware {
	...
	final Map<String, String> filePathToURI = new ConcurrentHashMap<>();
	...
	public static void main(String[] args) throws InterruptedException, ExecutionException {
		if (args.length == 0) return; // don't do anything
		if (args[0].equals("-lsp")) {
	        ...
	        start_listening.get();
		} else if (args[0].equals("-interp")) {
			// run the interpreter
			if (args.length <= 1) return; // don't do anything, can't run it!
			InterpreterPipeline.runOnFileName(args[1]);
