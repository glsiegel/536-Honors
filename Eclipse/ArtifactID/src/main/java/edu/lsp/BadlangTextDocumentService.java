...
import pipeline.FileManagerException;
...
public class BadlangTextDocumentService implements TextDocumentService {
	...
	public void didOpen(DidOpenTextDocumentParams params) {
		...
		try {
			...
			server.demoMessage("Unrecoverable error encountered with LSP.");
	...
	public void didChange(DidChangeTextDocumentParams params) {
		...
		if (sb == null) {
			...
			server.demoMessage("Could not find this text document.");
			return;
		}
	...
	public void runCheckProcedure(String uri, File f, String file_content) {
		...
		if (fileManager == null) {
			...
			try {
				...
				errors_aggregate.add(new error.ParserError(new CodeSection(0,0,0,0), "This file's \"as\" clause is incorrect. The file is not located at " + data.rootpath + ". Change the \"as\" clause, or change the file's name."));
		...
		checked_rootpaths.addAll(gb.rootpaths_to_pull_in);
		...
		while (rootpaths_to_pull.size() > 0) {
			...
			if (!other_rootpath.equals(other_data.rootpath)) {
				other_errors.add(new error.VisitorError(new CodeSection(0, 0, 0, 0),
							String.format("This file name does not match its declared name. Expect odd behavior. File name: %s, Declared name: %s", other_rootpath, other_data.rootpath)));
			}
			...
			for (Rootpath rootpath_to_add : other_gb.rootpaths_to_pull_in) {
