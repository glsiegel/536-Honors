...
class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
	...
	/**
	 * If this interpreter is ran on something with an "as" clause, then its
	 * rootpath is automatically stored here.
	 */
	private Rootpath this_rootpath;
	/**
	 * The file that this interpreter is running.
	 */
	private final File this_file;
	/**
	 * Current global environment Note that this can change when running code from
	 * other files.
	 */
	private GlobalEnvironment<Object> globals;
	/**
	 * Current environment. Could be global or local.
	 */
	private IEnvironment<Object> environment;
	/**
	 * Current environment depth. 0 is global.
	 */
	private int depth = 0;
	/**
	 * Statements that we are to run on.
	 */
	private List<Stmt> statementList;
	/**
	 * Storage space for return values of function calls.
	 */
	 ...
	 * 
	 * @param statements parsed statements of the file to run
	 * @param f          the file running, important for import reasons
	 */
	...
	/**
	 * Runs the interpreter on the provided statement list. Only run ONCE.
	 */
	public void run() {
	...
	/**
	 * Helper for executing a block.
	 * 
	 * @param statements block to execute
	 * @param given_env  lower-depth environment to switch to
	 */
	...
	public Void visitFunctionStmt(Stmt.Function stmt) {
		...
		if (globals.getFunction(fcnName) == null) {
			
			throw new RuntimeException("GlobalBuilder did not collect function '" + fcnName + "'.");
		}
	...
	public Void visitWhileStmt(Stmt.While stmt) {
		...
		// if it's not a function, use a different loop (so fewer redundant checks)
		if (!environment.isFunction()) {
	...
	/**
	 * Helper for calls. Allows both visitCallExpr and visitIndirectExpr to share
	 * code. Note that knowing the global context of the function is important.
	 * 
	 * @param expr        call part of the expression
	 * @param function    function referenced by the call
	 * @param new_globals global context of the function, which we will temporarily
	 *                    swap our globals to
	 ...
	 * 
	 * @param type  type to attempt to match
