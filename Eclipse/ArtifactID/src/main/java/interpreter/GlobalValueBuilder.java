 ...
 * Indirection is disallowed. This is a technical limitation. If not, then we
 * would have to check at the same time as constructing globals, which would
 * make dependency cycles much more difficult to manage. To get around this,
 * users may have to call special "setup" functions that will initialize globals
 * to intended values. I recognize that calls could be made to work, although I
 * chose to disallow this feature because it would move a lot of work into the
 * GlobalValueBuilder that I would prefer to keep elsewhere. As such, users will
 * have to deal with this slight limitation.
 * 
 * Additionally, note that the interpreter only RUNS one file. Other files are
 * not ran, rather their contents are referenced. Meaning, if other files run
 * code in global scope, that code will not be run. Only variable definitions
 * and functions are acknowledged.
 */
public class GlobalValueBuilder implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
	...
	public Object visitVariableExpr(Expr.Variable expr) {
		...
		// did not have the varaible
		visitorErrors
				.add(VisitorError.New(expr, "Tried to initialize a global with a variable that does not yet exist."));
		return 0;
	}
